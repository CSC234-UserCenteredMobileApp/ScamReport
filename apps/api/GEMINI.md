...
- **Middleware**: Cross-cutting plugins live in `src/core/middleware/`. Use `auth.middleware.ts` for Bearer ID token validation.
...
4. Add a test in `test/<name>.test.ts` (or co-located).
