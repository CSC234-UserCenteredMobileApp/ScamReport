...
async function toAskAiMessage(m: repo.PersistedMessage): Promise<AskAiMessage> {
  const attachments = await Promise.all(
    m.attachments.map(async (a) => {
      ...
      try {
        signedUrl = await getSignedUrl('chat-attachments', a.storagePath, 3600);
      } catch (err) {
        // Fall back to null URL — message body still renders.
        console.error('[ask-ai] sign-url-failed', { storagePath: a.storagePath, err });
      }
      return {
        id: a.id,
        mimeType: a.mimeType,
